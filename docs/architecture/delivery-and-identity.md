...
GitHub's token is exchanged through the GCP Workload Identity Federation provider, then impersonates the single `assessment-deployer` Google service account. No service-account key is created or stored. Repository variables contain only non-secret project, provider, service-account, state-bucket, region, HTTPS, DNS, and operator-identity configuration. The separately reviewed `GCP_CLUSTER_ADMIN_EMAIL` is the required 14th variable and is never committed.

The permanent operator is separate from that federated automation identity. Foundation grants the exact user Connect Gateway Reader/Admin and secret-scoped Grafana password access; Kubernetes binds the same exact user to `cluster-admin` in both clusters. The local helper rejects gcloud credential/access-token and service-account-impersonation overrides, forces Application Default Credentials off for the Gateway kubeconfig, and pins the selected user for credential generation and auth-plugin execution.

...
Normal teardown removes MCI/controller load-balancer resources, namespaces, clusters, NAT, address, Armor, BigQuery dataset/sink, secrets, optional DNS/certificate, and other platform/foundation resources. Foundation-only recovery can skip either a genuinely never-created platform or an exact empty platform bound to a prior completed teardown. Teardown intentionally retains the project, WIF pool/provider, deployer identity/IAM, enabled bootstrap APIs, versioned state bucket, any bootstrap/foundation/platform state versions, and the small completion marker. The live controller inventory is removed, but versioning and soft delete can retain its bound historical generations until lifecycle/retention expiry.

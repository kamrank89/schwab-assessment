...
Deployment is never automatic. `.github/workflows/deploy.yml` accepts only a manual dispatch on `main`, runs account-free validation first, then deploys foundation, platform/workloads, and smoke verification. The optional HPA and application-failover exercises default to `false`.
...
In the GitHub UI, choose **Actions → deploy → Run workflow**, select `main`, and leave both drill boxes clear. Record the workflow URL and commit SHA before collecting evidence.
...
make validate
  ...
  -> smoke verification and seven BigQuery SQL dry runs

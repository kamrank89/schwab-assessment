...
inspect_versioned_object() {
  ...
  local soft_deleted_error
  local soft_deleted_error_file="${temporary_root}/soft-deleted-list.err"
  ...
  if ! soft_deleted_json="$(gcloud --access-token-file="${adc_token_file}" storage ls \
    --soft-deleted --exhaustive --json "${bucket_objects_uri}" 2>"${soft_deleted_error_file}")"; then
    soft_deleted_error="$(<"${soft_deleted_error_file}")"
    if [[ "${soft_deleted_error}" == *"One or more URLs matched no objects."* ]]; then
      soft_deleted_json='[]'
    else
...
delete_orphaned_mcsd_firewalls() {
  local firewall_inventory
  local firewall_name
  local firewall_count=0

  firewall_inventory="$(gcloud compute firewall-rules list \
    --project="${PROJECT_ID}" --format=json 2>/dev/null)" ||
    die "Could not inspect residual Multi-Cluster Services firewall rules."
  jq -e 'type == "array"' <<<"${firewall_inventory}" >/dev/null ||
    die "Residual Multi-Cluster Services firewall inventory is malformed."

  while IFS= read -r firewall_name; do
    [[ -n "${firewall_name}" ]] || continue
    [[ "${firewall_name}" =~ ^gke-gke-assessment-(us-central1|us-east1)-[a-z0-9]+-mcsd$ ]] ||
      die "Residual Multi-Cluster Services firewall has an unexpected name."
    gcloud compute firewall-rules delete "${firewall_name}" \
      --project="${PROJECT_ID}" --quiet
    firewall_count=$((firewall_count + 1))
  done < <(jq -r --arg network "/projects/${PROJECT_ID}/global/networks/schwab-assessment" '
    .[] |
    select(
      (.name | type == "string") and
      (.name | test("^gke-gke-assessment-(us-central1|us-east1)-[a-z0-9]+-mcsd$")) and
  ...
  if ((firewall_count > 0)); then
    printf 'Deleted %d orphaned Multi-Cluster Services firewall rule(s).\n' "${firewall_count}"
  fi
}
